using Etna_Business.Services.Imp;
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Etna technical test", Version = "v1" });
...
builder.Services.AddDbContext<EtnaDbContext>(options =>
{
    options.UseSqlServer(builder.Configuration.GetConnectionString("EtnaConnection"));
});
...
{
    var Context = scope.ServiceProvider.GetRequiredService<EtnaDbContext>();
